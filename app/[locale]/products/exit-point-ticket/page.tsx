...
const keyFeatures = [
  { key: 'k1', icon: <LuTicket className="w-6 h-6 text-green-600" /> },
  { key: 'k2', icon: <LuScanLine className="w-6 h-6 text-green-600" /> },
...
const steps = [
  ...
  { key: 's2', icon: <LuTicket className="w-16 h-16 text-green-600" /> },
...
const specs = [
  ...
  { key: 'sp2', icon: <LuTicket className="w-4 h-4 text-gray-400" /> },
  { key: 'sp3', icon: <LuQrCode className="w-4 h-4 text-gray-400" /> },
  { key: 'sp4', icon: <LuRadioTower className="w-4 h-4 text-gray-400" /> },
...
const integrations = [
  { key: 'i1', icon: <LuTicket className="w-7 h-7 text-green-600" /> },
  { key: 'i2', icon: <LuScanLine className="w-7 h-7 text-green-600" /> },
...
const ecosystem = [
  ...
  { key: 'e2', href: '/products/pay-point', image: '/img/paypoint.png', icon: null },
  { key: 'e3', href: '/products/backoffice', image: '/img/back.jpg', icon: null },
];

export default async function ExitPointTicketPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations('ExitPointTicketPage');
  return (
    <main>
      {/* Navbar + Logo */}
              ...
              <div>
                ...
                <nav data-reveal className="flex items-center gap-2 text-sm text-gray-400 mb-6">
                  <Link href="/" className="flex items-center gap-1 hover:text-green-600 transition-colors">
                    <LuHouse className="w-4 h-4" /> {t('breadcrumb.home')}
                  </Link>
                  <LuChevronRight className="w-4 h-4" />
                  <Link href="/#products" className="hover:text-green-600 transition-colors">{t('breadcrumb.products')}</Link>
                  <LuChevronRight className="w-4 h-4" />
                  <span className="text-gray-700 font-medium">{t('breadcrumb.current')}</span>
                </nav>
                ...
                <h1 data-reveal data-reveal-delay="80" className="text-4xl sm:text-5xl md:text-6xl font-black text-gray-900 leading-tight mb-5">
                  {t.rich('heroTitle', { span: (chunks) => <span className="block text-green-500">{chunks}</span> })}
                </h1>
          ...
          <div className="grid lg:grid-cols-5 gap-8 items-stretch">
            <div data-reveal="fade-left" className="lg:col-span-3 grid sm:grid-cols-2 gap-6 items-center">
              <div className="relative bg-gray-50 rounded-2xl p-4 flex items-center justify-center">
                <div className="relative">
                  <Image src="/img/produs2 - Copy.png" alt="Exit Point Ticket componente" width={240} height={360} className="h-[360px] w-auto object-contain" />
                  {callouts.map((c) => (
                    <span
                      key={c.n}
              ...
              <ul className="space-y-4">
                {hardware.map((h) => (
                  <li key={h.key} className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-gray-900 font-bold text-sm leading-tight">{t(`hardware.${h.key}.title`)}</p>
                      <p className="text-gray-500 text-xs">{t(`hardware.${h.key}.desc`)}</p>
                    </div>
                  </li>
                ))}
              </ul>
