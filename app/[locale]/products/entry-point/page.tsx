...
export const metadata: Metadata = {
  ...
  keywords: [
    ...
    'echipament intrare parcare',
    'terminal tichete parcare',
    'ANPR parcare',
    'recunoaștere plăcuțe numere',
    'control acces intrare parcare',
    'validator tichete parcare',
    'barieră automată intrare',
    'emitere tichete parcare',
    'statie automata parcare intrare',
...
const keyFeatures = [
  { key: 'k1', icon: <LuQrCode className="w-6 h-6 text-green-600" /> },
  { key: 'k2', icon: <LuRadioTower className="w-6 h-6 text-green-600" /> },
  { key: 'k3', icon: <LuCreditCard className="w-6 h-6 text-green-600" /> },
  { key: 'k4', icon: <LuVolume2 className="w-6 h-6 text-green-600" /> },
  { key: 'k5', icon: <TbBarrierBlock className="w-6 h-6 text-green-600" /> },
  { key: 'k6', icon: <LuMonitor className="w-6 h-6 text-green-600" /> },
];

const steps = [
  { key: 's1', icon: <LuCar className="w-16 h-16 text-green-600" /> },
  { key: 's2', icon: <LuQrCode className="w-16 h-16 text-green-600" /> },
  { key: 's3', icon: <LuCreditCard className="w-16 h-16 text-green-600" /> },
  { key: 's4', icon: <FaParking className="w-14 h-14 text-green-600" /> },
  { key: 's5', icon: <TbBarrierBlock className="w-16 h-16 text-green-600" /> },
];
...
const integrations = [
  { key: 'i1', icon: <LuQrCode className="w-7 h-7 text-green-600" /> },
  { key: 'i2', icon: <LuRadioTower className="w-7 h-7 text-green-600" /> },
  { key: 'i3', icon: <LuCreditCard className="w-7 h-7 text-green-600" /> },
  ...
  { key: 'i7', icon: <LuLogOut className="w-7 h-7 text-green-600" /> },
  { key: 'i8', icon: <LuLayoutDashboard className="w-7 h-7 text-green-600" /> },
];

const ecosystem = [
  { key: 'e1', href: '/products/pay-point', image: '/img/paypoint.png', icon: null },
...
export default async function EntryPointPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations('EntryPointPage');
  return (
    <main>
      {/* Navbar + Logo */}
