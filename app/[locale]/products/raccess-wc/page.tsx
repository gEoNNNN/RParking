...
const heroFeatureKeys = ['h1', 'h2', 'h3', 'h4'];

const keyFeatures = [
  { key: 'k1', icon: <LuUsers className="w-6 h-6 text-green-600" /> },
  { key: 'k2', icon: <LuCreditCard className="w-6 h-6 text-green-600" /> },
  { key: 'k3', icon: <LuTrendingUp className="w-6 h-6 text-green-600" /> },
  { key: 'k4', icon: <LuShieldCheck className="w-6 h-6 text-green-600" /> },
];

const steps = [
  { key: 's1', icon: <LuUsers className="w-16 h-16 text-green-600" /> },
  { key: 's2', icon: <LuCreditCard className="w-16 h-16 text-green-600" /> },
  { key: 's3', icon: <LuCheck className="w-16 h-16 text-green-600" /> },
  { key: 's4', icon: <LuMonitor className="w-16 h-16 text-green-600" /> },
];

const components = [
  { n: 1, key: 'c1', image: '/img/wc/img 4.png' },
  { n: 2, key: 'c2', image: '/img/wc/img 2.png' },
  { n: 3, key: 'c3', image: '/img/wc/img 3.png' },
];

const locations = [
  { key: 'l1', icon: <LuHouse className="w-8 h-8 text-green-600" /> },
  { key: 'l2', icon: <LuBanknote className="w-8 h-8 text-green-600" /> },
  { key: 'l3', icon: <LuCreditCard className="w-8 h-8 text-green-600" /> },
  { key: 'l4', icon: <LuTicket className="w-8 h-8 text-green-600" /> },
  { key: 'l5', icon: <LuTrendingUp className="w-8 h-8 text-green-600" /> },
  { key: 'l6', icon: <LuSmartphone className="w-8 h-8 text-green-600" /> },
  { key: 'l7', icon: <LuReceipt className="w-8 h-8 text-green-600" /> },
];

const specs = [
  { key: 'sp1', icon: <LuMonitor className="w-4 h-4 text-gray-400" /> },
  { key: 'sp2', icon: <LuBanknote className="w-4 h-4 text-gray-400" /> },
  { key: 'sp3', icon: <LuCreditCard className="w-4 h-4 text-gray-400" /> },
  { key: 'sp4', icon: <LuSmartphone className="w-4 h-4 text-gray-400" /> },
  { key: 'sp5', icon: <LuUsers className="w-4 h-4 text-gray-400" /> },
  { key: 'sp6', icon: <LuZap className="w-4 h-4 text-gray-400" /> },
  { key: 'sp7', icon: <LuWifi className="w-4 h-4 text-gray-400" /> },
  { key: 'sp8', icon: <LuSettings className="w-4 h-4 text-gray-400" /> },
];

export default async function RAccessWCPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations('RAccessWCPage');
  return (
    <>
                  ...
                  {keyFeatures.map((f) => (
                    <div key={f.key} className="flex gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500 text-sm mt-1">{t(`key.${f.key}.desc`)}</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <section className="py-24 bg-white">
          <div className="max-w-screen-2xl mx-auto px-6 lg:px-10">
            <div className="text-center mb-16">
              ...
              <h2 className="text-4xl font-bold text-gray-900 mb-4">{t.rich('componentsTitle', { span: (chunks) => <span className="text-green-600">{chunks}</span> })}</h2>
              <p className="text-gray-500 max-w-2xl mx-auto">{t('componentsDesc')}</p>
            </div>
              ...
              {components.map((c, i) => (
                ...
                >
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    <h3 className="text-lg font-bold text-gray-900 mb-2">{t(`components.${c.key}.title`)}</h3>
                    <p className="text-gray-500 text-sm leading-relaxed flex-1">{t(`components.${c.key}.desc`)}</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              {steps.map((step, idx) => (
                <Fragment key={step.key}>
                  <div className="flex flex-col items-center text-center gap-3 lg:flex-1">
                    ...
                    <p className="text-gray-900 font-bold text-sm leading-tight max-w-48">{t(`steps.${step.key}.title`)}</p>
