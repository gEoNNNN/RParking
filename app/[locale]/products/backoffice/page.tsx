...
export const metadata: Metadata = {
  ...
  keywords: [
    ...
    'platforma cloud parcare',
    'monitorizare parcare timp real',
    'rapoarte parcare',
    'administrare tarife parcare',
    'BackOffice cloud parcare',
    'RParking BackOffice',
    'SaaS parcare',
    'sistem management parcare',
    'parcare online management',
    'echipament parcare',
...
const keyFeatures = [
  { key: 'k1', icon: <LuLayoutDashboard className="w-6 h-6 text-green-600" /> },
  { key: 'k2', icon: <LuFileText className="w-6 h-6 text-green-600" /> },
  { key: 'k3', icon: <LuUsers className="w-6 h-6 text-green-600" /> },
  { key: 'k4', icon: <LuTag className="w-6 h-6 text-green-600" /> },
  { key: 'k5', icon: <LuBell className="w-6 h-6 text-green-600" /> },
  { key: 'k6', icon: <LuShieldCheck className="w-6 h-6 text-green-600" /> },
];

const steps = [
  { key: 's1', icon: <LuServer className="w-16 h-16 text-green-600" /> },
  { key: 's2', icon: <LuCloud className="w-16 h-16 text-green-600" /> },
  { key: 's3', icon: <LuFileText className="w-16 h-16 text-green-600" /> },
  { key: 's4', icon: <LuBell className="w-16 h-16 text-green-600" /> },
  { key: 's5', icon: <LuSettings className="w-16 h-16 text-green-600" /> },
];

const modules = [
  { n: 1, key: 'm1', icon: <LuActivity className="w-5 h-5 text-green-600" /> },
  { n: 2, key: 'm2', icon: <LuFileText className="w-5 h-5 text-green-600" /> },
  { n: 3, key: 'm3', icon: <LuWallet className="w-5 h-5 text-green-600" /> },
  { n: 4, key: 'm4', icon: <LuUsers className="w-5 h-5 text-green-600" /> },
  { n: 5, key: 'm5', icon: <LuSettings className="w-5 h-5 text-green-600" /> },
];

const integrations = [
  { key: 'i1', icon: <LuLogIn className="w-7 h-7 text-green-600" /> },
  { key: 'i2', icon: <LuLogOut className="w-7 h-7 text-green-600" /> },
  { key: 'i3', icon: <LuWallet className="w-7 h-7 text-green-600" /> },
  { key: 'i4', icon: <TbBarrierBlock className="w-7 h-7 text-green-600" /> },
  { key: 'i5', icon: <LuCamera className="w-7 h-7 text-green-600" /> },
  { key: 'i6', icon: <LuCreditCard className="w-7 h-7 text-green-600" /> },
  { key: 'i7', icon: <LuCpu className="w-7 h-7 text-green-600" /> },
  { key: 'i8', icon: <LuBuilding2 className="w-7 h-7 text-green-600" /> },
];

const security = [
  { key: 'sec1', icon: <LuLock className="w-5 h-5 text-green-400" /> },
  { key: 'sec2', icon: <LuDatabase className="w-5 h-5 text-green-400" /> },
  { key: 'sec3', icon: <LuClock className="w-5 h-5 text-green-400" /> },
  { key: 'sec4', icon: <LuShieldCheck className="w-5 h-5 text-green-400" /> },
];
...
export default async function BackOfficePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations('BackofficePage');
  return (
    <main>
      {/* Navbar + Logo */}
      ...
      <section className="py-16" style={{ backgroundColor: '#f7f9f4' }}>
        <div className="max-w-screen-2xl mx-auto px-6 lg:px-10">
          <div data-reveal="fade" className="mb-12">
            <h2 className="text-3xl font-bold text-gray-900 mb-3">{t('modulesTitle')}</h2>
            <div className="w-12 h-0.5 bg-green-500" />
          </div>
          ...
          <div className="grid lg:grid-cols-5 gap-8 items-center">
            ...
            <div data-reveal="fade-left" className="lg:col-span-2 space-y-4">
              {modules.map((m) => (
                <div key={m.key} className="flex items-start gap-4 bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 p-5">
                  <div className="w-10 h-10 rounded-xl bg-green-50 flex items-center justify-center shrink-0">
                    {m.icon}
                  </div>
                  <div>
                    <p className="text-gray-900 font-bold text-sm leading-tight mb-1">{t(`modules.${m.key}.title`)}</p>
                    <p className="text-gray-500 text-xs leading-relaxed">{t(`modules.${m.key}.desc`)}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="max-w-screen-2xl mx-auto px-6 lg:px-10">
          <div className="grid lg:grid-cols-2 gap-10">
            ...
            <div data-reveal="fade-right">
              <div className="mb-8">
                <h2 className="text-3xl font-bold text-gray-900 mb-3">{t('securityTitle')}</h2>
                <div className="w-12 h-0.5 bg-green-500" />
              </div>
                  ...
                  <ul className="space-y-4">
                    {security.map((s) => (
                      <li key={s.key} className="flex items-center gap-3">
                        {s.icon}
                        <span className="text-gray-200 text-sm font-medium">{t(`security.${s.key}`)}</span>
                      </li>
                    ))}
                  </ul>
                </div>
